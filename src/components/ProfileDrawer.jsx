...
export default function ProfileDrawer({
  ...
  const content = (
    <>
      ...
      />
      <aside className="fixed left-0 top-0 bottom-0 z-[110] w-[min(88vw,340px)] bg-card border-r border-border shadow-2xl overflow-hidden flex flex-col">
  ...
  if (isNative) return open ? content : null;
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          />
          <motion.aside
            className="fixed left-0 top-0 bottom-0 z-[110] w-[min(88vw,340px)] bg-card border-r border-border shadow-2xl overflow-hidden flex flex-col"
            initial={{ x: '-100%' }}
            animate={{ x: 0 }}
            exit={{ x: '-100%' }}
